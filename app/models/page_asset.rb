class PageAsset < ApplicationRecord
  ...
  def file_container
    @file_container ||= FileContainer.new(File.join('source', 'assets', page.id, name))
  end
  ...
  def self.create_from_uri!(uri, page:)
  ...
  def self.create_from_base64_uri!(uri, page:)
  ...
  def self.create_from_http_uri!(uri, page:)
    uri      = URI(uri)
